...
description: "Use when running a thorough public-release audit. Applies the core release gate, follows the exhaustive review protocol, writes a findings file, and then hands off to fix-and-recheck."
tools:
  - read
  - search
  - edit
...
- Use this agent as the coordination layer: run the workflow, keep the findings file authoritative, and preserve the prompt-defined audit procedure instead of restating it here.

After the audit is complete, recommend running `fix-and-recheck` next so the findings file can be consumed and resolved in severity order.